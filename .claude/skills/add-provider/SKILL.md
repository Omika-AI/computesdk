...
name: add-provider
description: >-
  Guide a provider author through adding their compute provider to ComputeSDK and
  opening a PR. Use when someone wants to contribute a new `@computesdk/<name>`
  provider package, integrate their sandbox/compute service, or asks "how do I add
  my provider" / "open a PR to add <X>". Scaffolds the package, implements the
  provider against the framework, verifies build/typecheck/lint/tests, adds a
  changeset, and opens the pull request.
---

# Add a Provider to ComputeSDK

Help a provider author ship a new `@computesdk/<name>` package and open a PR against
`computesdk/computesdk`. The canonical implementation spec lives in
[ADD-PROVIDER.md](../../../ADD-PROVIDER.md) — read it before scaffolding and treat it
as the source of truth for file contents, method signatures, and naming. This skill
is the orchestration layer around that doc: it gathers what's needed, drives the
edits, verifies, and handles the git/PR mechanics.
...
- **Match the codebase.** Copy structure and conventions from an existing reference
  provider rather than inventing them. Good references:
